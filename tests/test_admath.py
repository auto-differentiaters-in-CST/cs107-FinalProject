...
def test_choose():
    ...
    assert choose(10,1) == 10, "Error: choose function doesn't calculate correctly."
    assert choose(9,9) == 1, "Error: choose function doesn't calculate correctly."
    assert choose(3,2) == 3, "Error: choose function doesn't calculate correctly."
    with pytest.raises(Exception):
        choose(1,-1)
        choose(5,11)
        choose(4.5,1.1)
...
def test_fact_ad():
    assert fact_ad(2,0) == 1, "Error: fact_ad calculation wrong."
    assert fact_ad(2,2) == 2, "Error: fact_ad calculation wrong."
    assert fact_ad(2,3) == 0, "Error: fact_ad calculation wrong."
    assert fact_ad(3.5,2) == 3.5*2.5, "Error: fact_ad calculation wrong."
...
def test_sin_ad():
    ...
    assert np.allclose(sin(x+y).diff(),np.array([np.cos(3),np.cos(3)])), "Error: sin(x+y), false derivative."
    f = sin(VAD.VAD([1,2]))
...
def test_cos_ad():
    ...
    assert np.allclose(cos(x+y).diff(),np.array([-np.sin(3),-np.sin(3)])), "Error: cos(x+y), false derivative."
    f = cos(VAD.VAD([1,2]))
...
def test_tan_ad():
    ...
    assert np.allclose(tan(x+y).diff(),np.array([sec(3)**2,sec(3)**2])), "Error: tan(x+y), false derivative."
    f = tan(VAD.VAD([1,2]))
    assert np.allclose(f.val,np.array([[np.tan(1)],[np.tan(2)]])), "Error: tan(VAD[1,2]), false value."
    der1 = np.zeros((2,2))
    assert np.allclose(tan(x+y).der2,np.array([[2*tan(3)*sec(3)**2,2*tan(3)*sec(3)**2],[2*tan(3)*sec(3)**2,2*tan(3)*sec(3)**2]])), "Error: tan(x+y), false der2."
    der1[0,0] = sec(1)**2
    der1[1,1] = sec(2)**2
    der2 = np.zeros((2,2,2))
...
def test_sec_float():
    ad = 2
    assert abs(sec(ad)-1/math.cos(2)) <= 1e-8, "Error: sec(x), false value."
...
def test_sinh_ad():
    ...
    assert np.allclose(sinh(x+y).diff(),np.array([np.cosh(3),np.cosh(3)])), "Error: sinh(x+y), false derivative."
    f = sinh(VAD.VAD([1,2]))
...
def test_cosh_ad():
    ...
    assert np.allclose(cosh(x+y).diff(),np.array([np.sinh(3),np.sinh(3)])), "Error: cosh(x+y), false derivative."
    f = cosh(VAD.VAD([1,2]))
...
def test_tanh_ad():
    ...
    assert np.allclose(tanh(x+y).diff(),np.array([sech(3)**2,sech(3)**2])), "Error: tanh(x+y), false derivative."
    assert np.allclose(tanh(x+y).der2,np.array([[-2*tanh(3)*sech(3)**2,-2*tanh(3)*sech(3)**2],[-2*tanh(3)*sech(3)**2,-2*tanh(3)*sech(3)**2]])), "Error: tanh(x+y), false der2."
    f = tanh(VAD.VAD([1,2]))
...
def test_sech_float():
    ad = 2
    assert abs(sech(ad)-1/np.cosh(2)) <= 1e-8, "Error: sech(x), false value."
